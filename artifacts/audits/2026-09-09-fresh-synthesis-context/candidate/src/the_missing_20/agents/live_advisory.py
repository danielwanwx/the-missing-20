...
class AdvisoryValidationError(ValueError):
    ...
    def __init__(self, message: str) -> None:
        ...
        self.fresh_synthesis_context: dict[str, Any] = {}
...
class _FreshSynthesisDiagnosticCapture:
    """Keep one raw provider value for the private diagnostic artifact only."""

    _first_structured_raw_value: Any = field(default=None, repr=False)
    _received: bool = False

    def record(self, value: Any) -> None:
        if self._received:
            return
        self._first_structured_raw_value = copy.deepcopy(value)
        self._received = True

    def private_metadata(self) -> dict[str, Any]:
        return {
            "status": "UNVALIDATED" if self._received else "NOT_RECEIVED",
            "first_structured_raw_value": (
                copy.deepcopy(self._first_structured_raw_value) if self._received else None
            ),
        }


_FRESH_SYNTHESIS_CAPTURE_ATTRIBUTE = "_fresh_synthesis_diagnostic_capture"
...
class AdvisoryRun:
    ...
    fresh_synthesis_context: dict[str, Any] = field(default_factory=dict)
...
def _diagnostic_wall_timeout_seconds(factory: Any) -> float:
    """Keep the diagnostic's fresh Agent inside the original run deadline."""

    configured = getattr(
        getattr(getattr(factory, "config", None), "budget", None), "whole_run_timeout_seconds", None
    )
    if isinstance(configured, (int, float)) and not isinstance(configured, bool) and configured > 0:
        return min(ADVISORY_WALL_TIMEOUT_SECONDS, float(configured))
    return float(ADVISORY_WALL_TIMEOUT_SECONDS)
...
async def _invoke(
    ...
    *,
    ...
    continue_requested: Callable[[], bool] = lambda: True,
    _fresh_synthesis_context_diagnostic: bool = False,
    _fresh_synthesis_capture: _FreshSynthesisDiagnosticCapture | None = None,
) -> AdvisoryRun:
    try:
    ...
    if _fresh_synthesis_context_diagnostic and not source_investigation:
        raise AdvisoryValidationError(
            "fresh synthesis context diagnostic requires a source-investigation packet"
        )
    if _fresh_synthesis_context_diagnostic and delegation_journal is not None:
        raise AdvisoryValidationError(
            "fresh synthesis context diagnostic is limited to the single-agent workflow"
        )
    correlated_findings = (
    ...
    fresh_synthesis_context: dict[str, Any] = {}
    wall_timeout_seconds = (
        _diagnostic_wall_timeout_seconds(factory)
        if _fresh_synthesis_context_diagnostic
        else float(ADVISORY_WALL_TIMEOUT_SECONDS)
    )

    def attach_fresh_synthesis_context(
        error: AdvisoryUnavailable | AdvisoryValidationError,
    ) -> None:
        if _fresh_synthesis_context_diagnostic and fresh_synthesis_context:
            error.fresh_synthesis_context = dict(fresh_synthesis_context)
    ...
    try:
        with contextlib.redirect_stdout(io.StringIO()):
            if source_investigation or packet.get("case_class") == "receiving_operations":
                ...
                for _ in range(2):
                    ...
                    await asyncio.wait_for(
                        ...
                        timeout=max(0.01, wall_timeout_seconds - (time.perf_counter() - started)),
                    )
            ...
            if _fresh_synthesis_context_diagnostic:
                remaining_before_synthesis = wall_timeout_seconds - (time.perf_counter() - started)
                if remaining_before_synthesis <= 0:
                    raise AdvisoryUnavailable(
                        "fresh synthesis context diagnostic exhausted its shared run deadline"
                    )
                try:
                    context = build_fresh_synthesis_context(
                        ...
                        expected_payloads={
                            name: payloads[name]
                            for name in (*SOURCE_TOOL_NAMES, CORRELATION_TOOL_NAME)
                        },
                        required_tool_names=(*SOURCE_TOOL_NAMES, CORRELATION_TOOL_NAME),
                        prohibited_tool_names=(POLICY_TOOL_NAME,),
                    )
                except FreshSynthesisContextError as context_error:
                    unavailable = AdvisoryUnavailable(
                        "fresh synthesis context diagnostic cannot preserve the admitted read "
                        "boundary"
                    )
                    unavailable.diagnostics = [
                        {
                            "stage": "fresh_synthesis_context",
                            "case_id": packet.get("case_id"),
                            "failure": str(context_error),
                        }
                    ]
                    unavailable.fresh_synthesis_context = {
                        "first_candidate_only": True,
                        "same_factory_ledger": True,
                        "shared_deadline_seconds": wall_timeout_seconds,
                    }
                    raise unavailable from context_error
                fresh_synthesis_context = {
                    **context.audit_metadata(),
                    "first_candidate_only": True,
                    "same_factory_ledger": True,
                    "shared_deadline_seconds": wall_timeout_seconds,
                    "remaining_seconds_before_synthesis": remaining_before_synthesis,
                    "system_prompt_sha256": hashlib.sha256(
                        system_prompt.encode("utf-8")
                    ).hexdigest(),
                    "structured_output_schema_sha256": hashlib.sha256(
                        json.dumps(
                            LiveAdvisoryResult.model_json_schema(),
                            ensure_ascii=False,
                            sort_keys=True,
                            separators=(",", ":"),
                        ).encode("utf-8")
                ...
                agent = Agent(
                    model=factory.create(stage=AgentStage.SYNTHESIS, output_payload={}),
                    tools=[],
                    messages=cast(Any, copy.deepcopy(context.messages)),
                    **shared_agent_kwargs,
                )
        ...
        attach_fresh_synthesis_context(exc)
        raise
        ...
        attach_fresh_synthesis_context(unavailable)
    ...
    if (
        _fresh_synthesis_context_diagnostic
        and _fresh_synthesis_capture is not None
        and raw_result is not None
    ):
        _fresh_synthesis_capture.record(raw_result)
    ...
    if raw_result is None and stop_reason.startswith("limit_"):
        ...
        attach_fresh_synthesis_context(limited)
        raise limited
    if delegation is not None and (delegation.failed or not continue_requested()):
        ...
        attach_fresh_synthesis_context(stopped)
        raise stopped
    if isinstance(raw_result, LiveAdvisoryResult):
        ...
        try:
            ...
            invalid = AdvisoryValidationError("real advisory returned invalid structured data")
            invalid.usage = _usage_delta(before, factory.ledger.snapshot())
            attach_fresh_synthesis_context(invalid)
        ...
        attach_fresh_synthesis_context(error)
        raise error
    expected_raw = packet.get("expected_disposition")
    try:
        expected_disposition = AdvisoryDisposition(str(expected_raw))
    except ValueError as exc:
        invalid_packet = AdvisoryValidationError(
            "advisory packet has an invalid expected disposition"
        )
        invalid_packet.usage = _usage_delta(before, factory.ledger.snapshot())
        attach_fresh_synthesis_context(invalid_packet)
        raise invalid_packet from exc
    ...
    if not isinstance(expected_safe_next_step, str):
        invalid_packet = AdvisoryValidationError("advisory packet lacks expected safe next step")
        invalid_packet.usage = _usage_delta(before, factory.ledger.snapshot())
        attach_fresh_synthesis_context(invalid_packet)
    ...
    def check(candidate: LiveAdvisoryResult) -> None:
        if not continue_requested() or (delegation is not None and delegation.failed):
            stopped = AdvisoryUnavailable("parent investigation stopped or superseded")
            attach_fresh_synthesis_context(stopped)
            raise stopped
        read_ids = {
            item
    ...
    try:
        ...
        if _fresh_synthesis_context_diagnostic:
            error.fresh_synthesis_context = dict(fresh_synthesis_context)
            raise
        remaining = wall_timeout_seconds - (time.perf_counter() - started)
        if remaining <= 0:
            raise
    ...
    return AdvisoryRun(
        ...
        fresh_synthesis_context=fresh_synthesis_context,
    )
...
def run_fresh_synthesis_context_diagnostic(
    packet: Mapping[str, Any],
    *,
    factory: BedrockNovaProFactory,
    question: str,
    ...
    """Run the private, single-agent fresh-synthesis diagnostic once.

    This opt-in path preserves the normal advisory path.  It acquires actual
    source reads, then reconstructs a new synthesis Agent from only completed
    admitted tool pairs.  It never runs a repair turn.
    """
    ...
    try:
        ...
        capture = _FreshSynthesisDiagnosticCapture()
        try:
            return asyncio.run(
                _invoke(
                    packet,
                    ...
                    on_runtime_event=on_runtime_event,
                    continue_requested=continue_requested,
                    _fresh_synthesis_context_diagnostic=True,
                    _fresh_synthesis_capture=capture,
                )
            )
        except (AdvisoryUnavailable, AdvisoryValidationError) as error:
            # This stays outside normal advisory diagnostics and is read only by
            # the private probe when a first provider value failed validation.
            setattr(error, _FRESH_SYNTHESIS_CAPTURE_ATTRIBUTE, capture.private_metadata())
            raise
    raise AdvisoryUnavailable("fresh synthesis diagnostic cannot run inside an active event loop")
