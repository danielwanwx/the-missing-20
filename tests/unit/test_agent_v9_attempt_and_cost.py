...
def test_v9_attempt_claim_is_durable_exclusive_and_prose_free(tmp_path: Path) -> None:
    path = tmp_path / "artifacts/agent/bedrock-attempt-claim-v3.json"
    claim = claim_v9_attempt(path)
    assert load_v9_attempt_claim(path) == claim
    assert claim_digest(claim) == claim.claim_digest
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["agent_contract_version"] == "agent-contract/v9"
    assert payload["prior_cost_usd"] == 0.0814368
    assert payload["output_token_cap"] == 62_040
    assert payload["max_output_tokens_per_request"] == 1_551
    for forbidden in (str(tmp_path), "account", "credential", "password", "MODEL_PROSE"):
        assert forbidden not in path.read_text(encoding="utf-8")
    try:
        claim_v9_attempt(path)
    except V9AttemptAlreadyClaimed:
        pass
    else:  # pragma: no cover
        raise AssertionError("a second v9 claim must be refused")


def test_v9_concurrent_claimers_have_one_winner(tmp_path: Path) -> None:
    path = tmp_path / "claim.json"

    def attempt(_index: int) -> object:
        try:
            return claim_v9_attempt(path)
        except V9AttemptAlreadyClaimed:
            return None
...
def test_v9_budget_reservation_stays_below_cumulative_hard_cap() -> None:
