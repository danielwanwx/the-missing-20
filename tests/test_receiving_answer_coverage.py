...
def sources(count=2):
    return {
        "read_erp_evidence": {
            "uom": "Box",
            "quantities": {"ordered": 40, "physically_arrived": 1, "receipt_posted_quantity": 1},
            "records": [{"stock_entries": [{"voucher_no": "PR-5", "name": "SLE-9"}]}],
        },
        "read_operational_history": {
            "baseline": {
                "minimum_samples": 3,
                "metrics": {"received": {"sample_count": count, "status": "INSUFFICIENT_DATA"}},
            },
        },
    }
...
def test_compound_quantity_answer_cannot_omit_all_requested_numbers():
    gaps = receiving_answer_gaps(
        ...
        "Arrival one needs review and arrival two is posted.",
        sources(),
        None,
    )
    ...
    assert not receiving_answer_gaps(
        "Compare ordered, physically received and posted quantities.",
        "Ordered 40 Box, received 1 Box, posted 1 Box.",
        sources(),
        None,
    )
        ...
        LiveAdvisoryResult.model_validate(
            {
                ...
                "evidence_ids": ["PR-5"],
                "reason": "Receipt is posted.",
                "safe_next_step": "Inspect the receipt.",
