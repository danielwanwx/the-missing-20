    ...
    def transport(request: Request, _timeout: float) -> bytes:
        ...
        if path.startswith("/api/resource/Purchase%20Receipt/"):
            data = {
                ...
                "docstatus": 1,
                "supplier_delivery_note": "M20-DOCK-87421",
                "items": [
                    {
                        "item_code": "M20-ECU-CTRL",
                        "rejected_qty": 8,
                        "rejected_warehouse": "M20 Quality Hold - M20",
...
def test_delivery_idempotency_uses_sales_order_link_when_mapper_omits_remarks() -> None:
    def transport(request: Request, _timeout: float) -> bytes:
        path = request.full_url.split("https://erp.example", 1)[1]
        if path.startswith("/api/resource/Delivery%20Note?"):
            return json.dumps({"data": [{"name": "DN-M20-20"}]}).encode()
        if path == "/api/resource/Delivery%20Note/DN-M20-20":
            return json.dumps(
                {
                    "data": {
                        "name": "DN-M20-20",
    ...
    existing = executor._existing_order_effect(
        "Delivery Note",
        remarks="M20 DEMO customer delivery M20-CASE-20",
        customer="M20 Fleet",
        sales_order="SO-M20-20",
    )

    assert existing == "DN-M20-20"
