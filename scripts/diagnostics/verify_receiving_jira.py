...
def main() -> None:
    ...
    with sqlite3.connect(f"file:{journal_path}?mode=ro", uri=True) as db:
        rows = [
            ...
            for key, payload, raw in db.execute(
                "SELECT key,payload,state FROM handoffs "
    ...
    for key, event, retained in rows:
        ...
        proofs.append(
            {"key": key, "operation": event["operation"], "version": event.get("version"), **proof}
        )
        ...
        (
            {
                ...
                "body_sha256": hashlib.sha256(
                    json.dumps(row["body"], sort_keys=True).encode()
                ).hexdigest(),
            }
    ...
    snapshot = {
        ...
        "url": jira.origin + "/browse/" + issue["key"],
        "proofs": proofs,
