...
| Complete read-only workspace and fail-closed degraded/invalid modes | `PROVEN` | PASS | Browser smoke renders complete, degraded, and unavailable views with no active write control or remote resource. | `artifacts/workspace/browser-smoke-v1.json` |
...
| Model authority to grant, execute, verify, or replay an operational action | `PROVEN` | PASS | Advisory records carry the no-write boundary; deterministic code and two humans own controlled effects. | `artifacts/agent/authority-b-advisory-v1.json`, `artifacts/workspace/authority-b-lifecycle-v1.json` |
